...
Optionally, the same setup can resolve a Twilio Conversation Memory profile before dialing LiveKit. In that mode, Twilio passes Memory identifiers to LiveKit as SIP headers, and the LiveKit agent gets a `recall_customer_memory` tool it can call on demand after the caller describes the issue.
...
Optional Conversation Memory values:
...
`MEMORY_STORE_ID` is only needed if you want to use `/voice_memory`, `/studio_voice_memory`, or `/memory_recall`. In deployed Twilio Functions, the account SID and auth token are usually available as `ACCOUNT_SID` and `AUTH_TOKEN`, but setting `TWILIO_ACCOUNT_SID` and `TWILIO_AUTH_TOKEN` explicitly is useful for local tests or runtimes that do not expose the defaults.
...
- optional `X-Customer-Phone` -> `customerPhone`
...
- If you are enabling Conversation Memory for Pattern A, use `/studio_voice_memory` instead.
...
Use the same LiveKit tool shape as Pattern B, but point the handoff service call at `/studio_escalate` instead of `/escalate`. If you are enabling Conversation Memory, use `/studio_escalate_memory`.
...
If you enable Conversation Memory for Pattern B, use:

```text
POST https://your-functions-service-1234.twil.io/voice_memory
```
...
For Python, keep the default helper path so the tool posts to `/escalate`, or set `HANDOFF_ESCALATE_PATH=/escalate_memory` when using Conversation Memory:
...
Use `/escalate_memory` instead of `/escalate` when you want Memory identifiers copied into the TaskRouter task attributes.

## 6. Optional Conversation Memory

Use Conversation Memory when you want Twilio to maintain customer context across calls while LiveKit remains the real-time voice agent.
