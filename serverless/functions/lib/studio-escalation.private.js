async function handleStudioEscalation(
  context,
  event,
  callback,
  ...
  if (includeMemoryAttributes) {
    returnParams.customerPhone = normalizedText(payload.customerPhone, "", 40);
    returnParams.memoryStoreId = normalizedText(payload.memoryStoreId, "", 120);
    returnParams.memoryProfileId = normalizedText(payload.memoryProfileId, "", 120);
  }

  let studioReturnUrl;
  try {
    studioReturnUrl = buildStudioReturnUrl(context.STUDIO_FLOW_WEBHOOK_URL, returnParams);
  } catch (error) {
    response.setStatusCode(500);
    response.setBody({
